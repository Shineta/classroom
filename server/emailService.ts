...
${isLate ? 'This lesson plan was submitted after the Friday deadline.' : 'This lesson plan was submitted on time.'}
  ...
  // Send lesson plan review notification to reviewers
  async sendLessonPlanReviewNotification(reviewerEmail: string, data: {
    reviewerName: string;
    teacherName: string;
    lessonTitle: string;
    subject: string;
    ...
    try {
      const html = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #059669; margin-bottom: 20px;">New Lesson Plan Ready for Review</h2>
          
          <p>Dear ${data.reviewerName},</p>
          
          <p>A new lesson plan has been submitted and is ready for your review.</p>
          
          <div style="background-color: #f0fdf4; border-left: 4px solid #059669; padding: 16px; margin: 20px 0;">
            <h3 style="margin: 0 0 10px 0; color: #047857;">Lesson Plan Details</h3>
            <p style="margin: 5px 0;"><strong>Teacher:</strong> ${data.teacherName}</p>
            <p style="margin: 5px 0;"><strong>Title:</strong> ${data.lessonTitle}</p>
            <p style="margin: 5px 0;"><strong>Subject:</strong> ${data.subject}</p>
            <p style="margin: 5px 0;"><strong>Grade Level:</strong> ${data.gradeLevel}</p>
            <p style="margin: 5px 0;"><strong>Scheduled Date:</strong> ${data.dateScheduled}</p>
          </div>
          
          <p>Please review the lesson plan and provide feedback to support instructional quality and alignment with curriculum standards.</p>
          ...
          <p>Thank you for your commitment to supporting excellent instruction in our schools.</p>
          
          <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 30px 0;">
          <p style="color: #6b7280; font-size: 12px;">
            This notification was generated by <a href="https://hortontech.dev" style="color: #059669;">Horton Tech Innovations</a>
          </p>
        </div>
      `;

      const text = `
New Lesson Plan Ready for Review

Dear ${data.reviewerName},

A new lesson plan has been submitted and is ready for your review.
...
- Scheduled Date: ${data.dateScheduled}

Please review the lesson plan and provide feedback to support instructional quality and alignment with curriculum standards.

Review the lesson plan at: ${data.reviewUrl}

Thank you for your commitment to supporting excellent instruction in our schools.
      `;

      await mailService.send({
        to: reviewerEmail,
        from: process.env.FROM_EMAIL || 'noreply@classroom-walkthrough.com',
        subject: `New Lesson Plan Review: ${data.lessonTitle}`,
        text,
        html,
      });

      console.log(`Lesson plan review notification sent to ${reviewerEmail}`);
      return true;

    } catch (error) {
      console.error("Failed to send lesson plan review notification:", error);
      return false;
    }
  }
}
